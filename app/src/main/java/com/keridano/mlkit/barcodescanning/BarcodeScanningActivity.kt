package com.keridano.mlkit.barcodescanning
        ...
        for ((index, barcode) in barcodes.withIndex()) {
            ...
            when (barcode.valueType)
            {
                FirebaseVisionBarcode.TYPE_WIFI -> {
                    ...
                    barcodeValue = "SSID: $ssid PASSWORD: $password ENCRIPTIONTYPE: $type"
                }
                FirebaseVisionBarcode.TYPE_URL -> {
                    val title = barcode.url!!.title
                    val url = barcode.url!!.url

                    barcodeValue = "URL: $title $url "
                }
                FirebaseVisionBarcode.TYPE_EMAIL -> {
                    val mailto = barcode.email!!.address
                    val subject = barcode.email!!.subject
                    val body = barcode.email!!.body
                    val type = barcode.email!!.type

                    barcodeValue = "MAILTO: $mailto\nSUBJECT: $subject\nTYPE: $type\nBODY: $body"
                }

            }
            barcodeScanningModels.add(BarcodeScanningModel(index, barcodeValue))
        }
    }
