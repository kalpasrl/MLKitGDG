...
class FaceDetectionActivity : AppCompatActivity(), FrameProcessor {
    ...
    override fun process(frame: Frame) {
        ...
        try {
            ...
            runOnUiThread {
                ...
                face_detection_camera_image_view.setImageBitmap(null)
                face_detection_camera_container.visibility = View.VISIBLE
            }
        }
    }
    ...
    private fun detectFaces(faces: List<FirebaseVisionFace>?, image: Bitmap?) {
        ...
        for ((index, face) in faces.withIndex()) {
            ...
            canvas.drawText("Face$index",
                    (face.boundingBox.centerX() - face.boundingBox.width() / 2) + 8F,
                    (face.boundingBox.centerY() + face.boundingBox.height() / 2) - 8F, faceTextPaint)
            ...
            faceDetectionModels.add(FaceDetectionModel(index, "Smiling Probability  ${"%.2f".format(face.smilingProbability.times(100))}%"))
            faceDetectionModels.add(FaceDetectionModel(index, "Left Eye Open Probability  ${"%.2f".format(face.leftEyeOpenProbability.times(100))}%"))
            faceDetectionModels.add(FaceDetectionModel(index, "Right Eye Open Probability  ${"%.2f".format(face.rightEyeOpenProbability.times(100))}%"))
        }
    }
